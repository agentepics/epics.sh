...
func (Adapter) InstallDir(cwd, slug string) string {
	return filepath.Join(cwd, ".gemini", "skills", slug)
}

func (Adapter) Setup(cwd string) (hostapi.Result, error) {
	files := map[string]string{
		filepath.Join(".gemini", "commands", "epics-resume.md"): resumeCommand(),
		filepath.Join(".gemini", "commands", "epics-info.md"):   infoCommand(),
		filepath.Join(".gemini", "commands", "epics-doctor.md"): doctorCommand(),
	}
	...
	state, err := hostutil.AppendSection(filepath.Join(cwd, "GEMINI.md"), guidance())
	if err != nil {
		return hostapi.Result{}, err
	}
	hostutil.RecordWrite(&result, "GEMINI.md", state)

	return result, nil
}

func (Adapter) Doctor(cwd string) ([]doctor.Check, error) {
	return hostutil.HostDoctorChecks(cwd, "gemini", ".gemini", "GEMINI.md", epicGuidanceMarker)
}

func guidance() string {
	return epicGuidanceMarker + "\n\n" +
		"This workspace uses the `epics` CLI as the canonical Epic control surface.\n\n" +
		"Use these Gemini command prompts when they are available:\n\n" +
		...
		"Keep Epic semantics in the package files and use Gemini-specific files only as wrappers around the CLI."
}
